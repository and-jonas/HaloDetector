...
def get_color_spaces(patch):
    ...
    # SCI = np.array((r-g)/(r+g), dtype=np.float32)
    # GLI = np.array((2*g - r - b)/(2*g + r + b), dtype=np.float32)
    # HI = np.array((2*r - g - b)/(g - b), dtype=np.float32)
    # NGRDI = np.array((g - r)/(g + r), dtype=np.float32)
    # SI = np.array((r-b)/(r+b), dtype=np.float32)
    # HUE = np.array(math.atan(2*((b - g - r)/30.5)*(g - r)))

    # Concatenate all
    descriptors = np.concatenate(
        [img_RGB, img_HSV, img_Lab, img_Luv, img_YUV, img_YCbCr, np.stack([ExG, ExR, TGI],
                                                                          axis=2)], axis=2)
    # Names
...
def cluster_complete_profiles(data):
    ...
    if n_profs > 20:
        ...
        max_n_clust = int(np.ceil(data.shape[0] / 25))  # changed from 30
